...
test("open tabs refresh campaign truth and never overstate unfinished browser checks", () => {
  ...
  const postSignatureRecheck = app.indexOf("canContinueRecoveryAuthorization", signatureRequest);
  const submittedMarker = app.indexOf("releaseSubmitted = true", signatureRequest);
  ...
  assert.ok(postSignatureRecheck > signatureRequest);
  assert.ok(submittedMarker > postSignatureRecheck);
  assert.ok(releaseRequest > submittedMarker);
  ...
  const postSignatureGuard = app.slice(postSignatureRecheck, submittedMarker);
