...
Release order is API first, frontend second: verify the stable Render config exposes the canonical `publicOrigin` and `consent.freshReadAdmission: "anonymous-v1"` before publishing the config-bound frontend. The compatible frontend sends no fresh-read credential in that transition mode. Only after that pair is verified may a reviewed Cloudflare API cutover advertise `pair-signature-v1`, issue the Durable-Object HMAC receipt after live pair qualification, and require the same pair-bound wallet signature on forced-fresh reads. Rollback order is frontend/API compatibility first, then the prior API. This preserves compatibility because the prior frontend ignores the new field, while the hardened frontend deliberately rejects a prior config that cannot identify its fresh-read admission contract.

The Cloudflare implementation isolates `/health/recovery-v2` in a fixed observation-only object name under the existing `RECOVERY_CAMPAIGN` Durable Object binding; it does not share the pool-and-campaign object's provider budget, HMAC state, or fresh-read cadence, and it requires no new binding or migration. Its thirty-second persisted snapshot and pre-provider lease cap this anonymous integrity endpoint at one dual-provider observation per interval, including failure and object eviction. The cache identity includes the existing immutable `CF_VERSION_METADATA.id`, so a new Worker deployment cannot reuse and relabel the prior version's snapshot even if the optional Git revision is unavailable. Still supply the exact reviewed lowercase 40-character Git commit as `RETRYCREDIT_DEPLOYMENT_REVISION` so public rollout metadata and source identity remain auditable. The cache is an upstream-capacity boundary, not evidence that the Cloudflare API has been cut over or that unrelated-user completion has occurred.
