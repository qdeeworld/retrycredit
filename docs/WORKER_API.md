...
- On the isolated Cloudflare read plane, `GET /api/recovery/config?fresh=1` first reserves one persisted five-second admission window in the existing pool-and-campaign Durable Object. The reservation is committed before service initialization or provider work and remains after provider failure or object eviction. A cold admitted refresh supplies the readiness state read instead of triggering a second campaign read. Additional forced refreshes inside that global campaign window return HTTP `429` / `RECOVERY_FRESH_READ_THROTTLED` with the remaining whole seconds in `Retry-After`; they never receive cached data represented as fresh. Ordinary configuration reads and other operations do not consume or extend this window. Storage failure or invalid persisted state fails closed with HTTP `503` before provider work. This is an upstream-cadence control, not caller authentication or fairness; promotion to a write-capable Cloudflare service remains blocked until fresh admission is bound to the existing pair-signature capability.
...
The optional exact query `?fresh=1` requests provider-backed campaign truth and cannot be satisfied from the normal short cache. On the isolated Cloudflare deployment it is globally duty-cycled per active pool and campaign as described above. The browser retries only this exact forced-refresh throttle, only when the response includes a valid integer `Retry-After`, and only inside its existing bounded configuration-wake budget. It waits at least the advertised delay plus at most 250 milliseconds of positive jitter and abandons later attempts if the active wallet/pair operation changed. If fresh truth still cannot be obtained after signing, the browser discards the stale qualifying verdict, sends no release request, preserves the entered pair, and offers the same pair check again.
